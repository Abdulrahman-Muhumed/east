...
// Locale-aware Link from your next-intl navigation wrapper
...
function mapFeatured(products) {
    return (products || []).map((p) => ({
        ...
        tagline:
            p?.tagline ||
            p?.category ||
            "Premium quality from East Africa",
...
export default function HomePage() {
    // Pull once; listProducts is assumed synchronous data util
                ...
                >
                    <div className="mx-auto flex w-full max-w-5xl flex-col md:flex-row items-center justify-between gap-6 rounded-2xl bg-slate-900/60 ring-1 ring-white/10 backdrop-blur shadow-soft">
                        ...
                        <div className="min-w-0 flex-1 p-4 md:p-8 flex flex-col">
                            ...
                            <p className="mt-2 line-clamp-3 text-slate-300/90 text-xs sm:text-sm">
                                {p.description}
                            </p>
                            ...
                            <div className="mt-2 flex flex-wrap gap-1.5">
                                {tags.map((t, i) => (
                                    <span
                                        key={i}
                                        className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[11px] text-slate-200"
                                    >
                                        {t}
                                    </span>
                                ))}
                            ...
                            <div className="mt-3 grid grid-cols-3 gap-2">
                                {chips.map((c, i) => (
                                    <div
                                        key={i}
                                        className="rounded-md bg-white/5 ring-1 ring-white/10 px-2 py-1.5 text-[11px] text-slate-200 text-center"
                                    >
                                        {c}
                                    </div>
                                ))}
    ...
    return (
        <section className="relative py-20 sm:py-24 px-4 sm:px-6 lg:px-8">
            <div
                ...
                style={{
                    background: `radial-gradient(600px circle at 50% 0%, ${YELLOW}, transparent 45%)`,
                }}
            />
            <div className="mx-auto max-w-7xl">
                <motion.header
...
function TestimonialCard({ quote, who, avatar }) {
    return (
        <div className="relative rounded-3xl h-full p-6 sm:p-8 backdrop-blur-md bg-white/70 ring-1 ring-slate-900/10 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.35)] hover:shadow-[0_8px_32px_-8px_rgba(0,0,0,0.4)] transition">
            <blockquote className="relative z-10 text-black">
                <p className="font-semibold text-lg sm:text-xl leading-snug tracking-tight">
                    &ldquo;{quote}&rdquo;
                </p>
