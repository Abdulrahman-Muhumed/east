...
const fadeUp = {
    ...
    show: { opacity: 1, y: 1, scale: 1, transition: { duration: 0.55, ease: [0.2, 0.7, 0.2, 1] } },
};
...
const item = {
    ...
    show: { opacity: 1, y: 1, transition: { duration: 0.45, ease: "easeOut" } },
};
...
export default function ProductDetailPage() {
            ...
            <div className="min-h-screen bg-white mt-30">
                <section className="flex items-center justify-center bg-neutral-50 p-4">
                    <div className="max-w-xl w-full text-center bg-white p-12 sm:p-16 rounded-3xl shadow-2xl border border-neutral-100 transform transition-all duration-300 hover:scale-[1.01]">
                        <div className="mb-8">
                            {/* SVG icon now uses the accent color */}
                            <svg
                            ...
                            {/* The main heading now uses the primary brand color */}
                            <h1 className="text-4xl sm:text-5xl font-extrabold" style={{ color: PRIMARY }}>
                                {t("notFound.title")}
                            </h1>
                        </div>
                        {/* The description text remains neutral for contrast */}
                        <p className="mt-4 text-lg text-neutral-500">
                            {t("notFound.desc")}
                        </p>

                        {/* The action button's background uses the primary brand color */}
                        <div className="mt-8">
                            <MockLink
                                href="/products"
    ...
    const keyFacts = [
        { icon: <Globe size={20} />, label: t("keyFacts.origin"), value: product.originCountries?.join(" • ") || "—" },
        { icon: <BoxIcon size={20} />, label: t("keyFacts.packaging"), value: product.packaging || "—" },
        {
            icon: <Star size={20} />,
    ...
    const normalizeLabel = (s) => {
        ...
        if (t0.includes("food") || t0.includes("beverage") || t0.includes("drink")) return "Food & beverage";
        if (t0.includes("pharma")) return "Pharmaceuticals";
        if (t0.includes("cosmetic")) return "Cosmetics";
        if (t0.includes("industrial") || t0.includes("ink") || t0.includes("adhesive")) return "Industrial Use";
    ...
    /* Images */
    const images = Array.isArray(product.images) && product.images.length ? product.images : ["/products/placeholder.jpg"];
    ...
    return (
        <div className="text-gray-900 md:mt-12 lg:mt-12 mt-4">
            ...
            <div className="container mx-auto p-2 sm:p-8 lg:p-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10">
                    ...
                    {/* RIGHT */}
                    <div className="">
                        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="mb-6">
                            <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight tracking-tight" style={{ color: PRIMARY }}>
                                {product.name}
                            </h1>
                        ...
                        >
                            {keyFacts.map((fact, i) => (
                                ...
                                >
                                    <div className="p-3 rounded-full flex items-center justify-center" style={{ background: `${ACCENT}22`, color: "#5b4b00" }}>
                                        {fact.icon}
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">{fact.label}</p>
                ...
                {/* Tabs */}
                <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.15 }} className="mt-8">
                    <div className="flex border-b border-gray-200">
                        <button
                            className={`px-4 py-3 font-semibold text-lg ${activeTab === "specifications" ? "text-gray-900 border-b-2" : "text-gray-500 hover:text-gray-700"}`}
                        ...
                        <button
                            className={`px-4 py-3 font-semibold text-lg ${activeTab === "applications" ? "text-gray-900 border-b-2" : "text-gray-500 hover:text-gray-700"}`}
                    ...
                    <div className="mt-6">
                        {activeTab === "specifications" && (
                            <motion.div id="tab-specs" role="tabpanel" variants={stagger(0.05)} initial="hidden" animate="show" className="space-y-4">
                                ...
                                {Object.entries(product.specs || {}).map(([key, value]) => (
                                    <motion.div variants={item} key={key} className="flex justify-between items-center pb-2 border-b border-gray-100">
                        ...
                        {activeTab === "applications" && (
                            <motion.div id="tab-apps" role="tabpanel" variants={fadeUp} initial="hidden" animate="show" className="prose text-gray-700 max-w-none">
                                <p className="mb-4">
                                    {product.specs?.applicationAreas ? t("tabs.appsLeadWithAreas") : t("tabs.appsLeadDefault")}
                                </p>
                                <motion.ul variants={stagger(0.07)} initial="hidden" animate="show" className="space-y-4 list-none p-0 mt-0">
                                    {applications.map((app) => {
                                        const label = t.optional?.("applications." + app) ? t("applications." + app) : app;
                                        return (
                                            <motion.li variants={item} key={app} className="flex items-center gap-3">
                ...
                {/* Related */}
                {related.length > 0 && (
                    <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.15 }} className="mt-16 border-t border-gray-200 pt-12">
                        ...
                        <div className="relative">
                            ...
                            <div ref={carouselRef} className="flex overflow-x-auto gap-6 pb-4 snap-x snap-mandatory scrollbar-hide select-none cursor-grab">
                                {related.map((rp) => (
                                    <div key={rp.id || rp.slug} className="snap-start flex-none w-[85%] sm:w-[48%] md:w-[32%]">
                                        <Link
                                        ...
                                        >
                                            ...
                                            <div className="mt-2 flex items-center gap-1 font-semibold" style={{ color: PRIMARY }}>
                ...
                {!sent ? (
                    <form onSubmit={submit} className="space-y-6 p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <Input label={t("fields.contactName")} value={form.contactName} onChange={(v) => setForm({ ...form, contactName: v })} required />
                            <Input label={t("fields.email")} type="email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} required />
                            <Input label={t("fields.destination")} value={form.destination} onChange={(v) => setForm({ ...form, destination: v })} />
                            <Input label={t("fields.quantity")} value={form.quantity} onChange={(v) => setForm({ ...form, quantity: v })} required />
                            <Select label={t("fields.unit")} value={form.unit} onChange={(v) => setForm({ ...form, unit: v })}>
                                <option>kg</option>
                                <option>ton</option>
                            </Select>
                            <Select label={t("fields.incoterm")} value={form.incoterm} onChange={(v) => setForm({ ...form, incoterm: v })}>
                                {(product?.incoterms || ["FOB", "CFR", "CIF"]).map((t0) => (
                                    <option key={t0}>{t0}</option>
                                ))}
                            </Select>
                        </div>
                        <Textarea label={t("fields.message")} rows={4} value={form.message} onChange={(v) => setForm({ ...form, message: v })} />

                        <div className="flex justify-end gap-3">
                            <button type="button" onClick={onClose} className="py-2 px-6 rounded-full border border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors">
                                {t("rfq.cancel")}
                            </button>
                            <button type="submit" disabled={sending} className="py-2 px-6 rounded-full text-gray-900 font-semibold relative">
                                <span className="relative z-10">{sending ? t("rfq.sending") : t("rfq.send")}</span>
                                <span
